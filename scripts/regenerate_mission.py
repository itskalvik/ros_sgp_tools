...
# Choose one set of waypoints
# (you might want to inspect available keys: list(waypoints.keys()))
selected_wp_set = list(waypoints.values())[0]  # use the first one
# Each row is likely [lat, lon, alt] or [x, y] in local frame

# Convert to GPS-style lat/lon if needed — assuming already [lat, lon]
...
# Step 3: Find TransectStyleComplexItem
...
# Step 4: Replace Items with new waypoints
new_items = []
base_id = 2  # Adjust if needed
...
for idx, (lat, lon) in enumerate(new_waypoints):
    new_item = {
        ...
        "params": [0, 0, 0, None, lat, lon, 50],  # Altitude 50m
        "type": "SimpleItem"
    }
...
# Step 5: Save updated mission plan
