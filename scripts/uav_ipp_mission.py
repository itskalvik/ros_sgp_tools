...
class IPPMissionPlanner(MissionPlanner):
    ...
    def mission(self):
        ...
        if self.set_home(self.vehicle_position[0], self.vehicle_position[1], self.mission_altitude):
            self.get_logger().info('Home position set')

        for i in range(len(self.waypoints)):
            self.current_waypoint.data = i+1
            self.get_logger().info(f'Visiting waypoint {i+1}')
            if self.go2waypoint([self.waypoints[i][0], self.waypoints[i][1], self.mission_altitude]):
